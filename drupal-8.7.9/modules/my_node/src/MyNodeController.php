...
class MyNodeController  extends ControllerBase {
  ...
  public function node_edit(Request $request) {
    ...
    while(isset($caption)) {
      $node = entity_load('node', $node->nid->value);
      $image_and_caption = $nodeModel->get_entity('field_image_and_caption', 'node', $node->nid->value, $i-1);

      \Drupal::logger('my_node')->error(print_r($image_and_caption,true));
      \Drupal::logger('my_node')->error(empty($image_and_caption));
      if($image_and_caption === false) {
        /*
        $paragraph = Paragraph::create(['type' => 'image_and_caption',]);
        $paragraph->set('field_caption', $caption); 
        $image_data = file_get_contents($field_image);
        ...
        */
        ...
        $parent_id = $image_and_caption[0]->field_image_and_caption_target_id;
        \Drupal::logger('my_node')->error("parent_id:".$parent_id);
        $caption = $request->query->get('caption'.$i);
        $nodeModel->update_paragraph__field_value("field_caption", $parent_id, $caption);
        if(isset($field_image) && strlen($field_image))  {
          \Drupal::logger('my_node')->error("file start");
          $image_data = file_get_contents($field_image);
          $file = file_save_data($image_data, "public://". basename($field_image).".jpg", FILE_EXISTS_REPLACE);
          \Drupal::logger('my_node')->error("file save");
          if($file->id()) {
            \Drupal::logger('my_node')->error("file execute start");
            $field_image = $nodeModel->get_paragraph__field("field_image", $parent_id);
            \Drupal::logger('my_node')->error(print_r($field_image, true));
            if($field_image !== false) {
              $nodeModel->update_paragraph__field_entity("field_image", $parent_id, $file->id());
              \Drupal::logger('my_node')->error("update_paragraph__field_entity");
            } else {
              $nodeModel->insert_paragraph__field_image("field_image", 'article', $parent_id, 0, $file->id());
              \Drupal::logger('my_node')->error("insert_paragraph__field_image");
            }
          }
        }
      }
