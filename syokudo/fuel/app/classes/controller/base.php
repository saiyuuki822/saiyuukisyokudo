...
class Controller_Base extends Controller_Template
{
  ...
  public $sub_menu;
    ...
    public function before()
    {
        parent::before();
        // メインナビゲーションを取得
        $curl = Request::forge('http://myportal.jpn.com/api/navigation/29', 'curl');
        $response = $curl->execute()->response();
        $list = \Format::forge($response->body,'json')->to_array();
        
        // サブナビゲーションを取得
        $curl = Request::forge('http://myportal.jpn.com/api/user_sub_menu/29', 'curl');
        $response = $curl->execute()->response();
        ...
        if(!empty($current_link)) {
          if($page[$current_link]['type'] == 2) {
              ...
              $curl = Request::forge('http://myportal.jpn.com/api/page/'. $tid, 'curl');
              $response = $curl->execute()->response();
        ...
        $this->template->sub_menu = View::forge('sub_menu', ["list" => $sub_menu_list]);
    }
